...
load("Myo_TGF_model_curves_20240505");
P_aa = 100:25:150;
...
plot(Try_Pf,Q_pred_cont_int/0.6,'r-','LineWidth',3);
plot(P_aa,train_data.Q_cont,'ko','LineWidth',2,'MarkerSize',10,'MarkerFaceColor','w');
...
subplot(1,3,2)
plot(Try_C_MD,P_GC_bell,'m-','LineWidth',3);hold on;
plot([25 bell_conc_points' 150],[bell_P_points(1)+3 bell_P_points' bell_P_points(3)],'ks','LineWidth',2,'MarkerSize',10,'MarkerFaceColor','w');hold on;
set(gca, 'box','off','FontSize',15);
xlim([0 200]);
ylim([40 65]);
xlabel("Macula Densa Osmolality (mOsmol/kg H_2O)",'FontSize',20);
ylabel("P_{G} (mmHg)",'FontSize',20);
axis square
...
plot(P_aa,train_data.Q_furo,'ko','LineWidth',2,'MarkerSize',10,'MarkerFaceColor','k');
plot(P_aa,train_data.Q_dilt,'kv','LineWidth',2,'MarkerSize',10,'MarkerFaceColor','w');
plot(bell_conc_points,bell_P_points,'ks','LineWidth',2,'MarkerSize',10,'MarkerFaceColor','w');

plot(Try_Pf,Q_pred_dilt/0.6,'k-','LineWidth',3);
plot(Try_Pf,Q_pred_furo/0.6,'b-','LineWidth',3);
plot(Try_Pf,Q_pred_cont/0.6,'m--','LineWidth',3);
plot(Try_Pf,Q_pred_cont_int/0.6,'r-','LineWidth',3);
plot(Try_C_MD,P_GC_bell,'m-','LineWidth',3);
hold off;
set(gca, 'box','off','FontSize',15);
xlim([90 160]);
ylim([1000 100000]);
...
legend('Control, Takenaka', ...
        'Furosemide, Takenaka', ...
        'Diltiazem, Takenaka', ...
        'Manual TGF, Bell',...
        'Model Diltiazem',...
        'Model Furosemide', ...
        'Model Control (No \Psi)', ...
        'Model Control (\Psi)', ...
        'Model Manual TGF',...
        'location','west')
axis off

   saveas(gcf,'figures/parameterization.fig')
   saveas(gcf,'figures/parameterization','epsc')
   saveas(gcf,'figures/parameterization','jpeg')
...
plot(Try_Pf,D_pred_dilt,'k-','LineWidth',3);hold on;
...
legend('Myogenic (No \Psi)','Myogenic (\Psi)','location','SouthEast')
...
label_1 = { 'Passive, 100','Passive, 130',...
                    'Control, 100','Control, 130'};
label = categorical(label_1);
label = reordercats(label,label_1);

strain_100 = strain_100([3 4],:)';
delt_shear_100 = delt_shear_100([3 4],:)';
delt_CSGFR_100 = delt_CSGFR_100([3 4],:)';
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
strain_130 = strain_130([3 4],:)';
delt_shear_130 = delt_shear_130([3 4],:)';
delt_CSGFR_130 = delt_CSGFR_130([3 4],:)';
...
color_mat = [0*strain_100(:,2) 0*strain_100(:,2) 0*strain_100(:,2)];
color_mat(:,1) = 1;
color_mat(:,2)= 1-(strain_130norm(:,i))/(max(max(strain_130norm)));
color_mat(:,3) = 1-(strain_130norm(:,i))/(max(max(strain_130norm)));%1-(strain_130norm(:,i))/(max(max(strain_130norm)));
...
text(p2.XData(195),p2.YData(195)-1.5,'EA','fontsize',20);
text(p2.XData(1),p2.YData(1)+1.5,'AA','fontsize',20);
if i==1
text(p2.XData(195)-2,p2.YData(1)+5,'Passive','fontsize',30);
else
text(p2.XData(195)-2,p2.YData(1)+5,'Control','fontsize',30);
end
set(gca,'Visible','off')

%color_mat = [0*strain_100(:,2) 0*strain_100(:,2) 0*strain_100(:,2)];
color_mat(:,1) = 1;
color_mat(:,2)= 1-(strain_130(:,1))/(max(max(strain_130)));
color_mat(:,3) = 1-(strain_130(:,1))/(max(max(strain_130)));

%1-(strain_130norm(:,i))/(max(max(strain_130norm)));
colormap(sort(color_mat,'descend'));
h=colorbar('FontSize',20);
caxis([min(min(strain_130)) max(max(strain_130))]);
