...
autoreg <- function(Pa_init,
  ...
  GG <- glom_SS(Pa=Paa_i,Ca=Ca_i,D=DAA_0)

  tau_del <- ceiling(tau_S/dt)
  tau_del_m <- ceiling(tau_m/dt)
  SNGFR_pre <- GG$SNGFR*c(rep(1,tau_del),0*Pa_input)
  Tp_pre <- DAA_0*(GG$Pavg-P_ext)/2*c(rep(1,tau_del_m),0*Pa_input)

  SNGFR_keep <- 0*Pa_input
  D_keep <- 0*Pa_input
  dDdt_keep <- 0*Pa_input
  C_MD_keep <- 0*Pa_input
  S_TGF_keep <- 0*Pa_input
  Tp_keep <- 0*Pa_input
  S_tone_keep <- 0*Pa_input
  S_SS_keep <- 0*Pa_input
  
  # tak_err_prs <- tak_err(
  #                 P_aa_input=Pa_init*rep(1,2),
  #                 D_aa_input=c(5,30),
  #                 Ca_input = Ca_input*rep(1,2),
  #                 PT_frac = PT_frac*rep(1,2),
  #                 C_0_Tub = C_0_Tub,
  #                 furo_yn=furo_yn,
  #                 SS_yn=SS_yn)
  
  D <- DAA_0#tak_err_prs$D_zer[1]
  Ca <- Ca_input[1]
  Pa <- Pa_init
  
  GG <- glom_SS(Pa=Pa,Ca=Ca,D=D)
  SNGFR <- GG$SNGFR
  print(GG)
  S_scale <- 1e6/60
  V_scale <- 10
  ...
  parameters <- c(SNGFR = SNGFR, PT_frac=PT_frac)
  x <- seq(0, L_Tub, by = L_Tub/1000)
  Ce <- C_0_Tub*(A1_Tub*(exp(-A3_Tub*x/L_Tub))+A2_Tub)
  C_temp <- ode(y = C_0_Tub, times = x, func = ss_Tubule, parms = parameters)
  C <- C_temp[,2]
  C_MD <- C[length(C)]
  ...
  for (i in seq(Pa_input)){
    ...
    Tp <- D*(Pavg - P_ext)/2
    Tp0 <- DAA_0*(glom_SS(Pa=Paa_i, Ca=Ca_i, D=DAA_0)$Pavg - P_ext)/2
    
    T_pass <- C_pass0*exp(C_pass1*(D/DAA_0-1))
    T_act <- C_act0*exp(-((D/DAA_0-C_act1)/C_act2)^2)
    S_TGF <- C_MD - C_MD_0 
    Tp_try <- Tp - Tp0
    S_SS <- Q/D^3 - glom_SS(Pa=Paa_i, Ca=Ca_i, D=DAA_0)$Q/DAA_0^3
    S_TGF_exp <- S_TGF^2/(S_TGF + exp(-5*S_TGF))
    
    if (furo_yn!=1){
      S_TGF_func <- C_TGF*S_TGF + C_TGF_tone#C_int*S_TGF*(Tp-Tp0) 
    }else{
      S_TGF_func <- 0
    }
    
    if (SS_yn!=0){
      S_int_func <-  C_SS_int_myo*S_SS*(Tp-Tp0) + C_SS_2*S_SS 
    }else{
      S_int_func <- 0
    }

    
    S_tone <- S_TGF_func + S_int_func + 
                  C_myo*(Tp_pre[i] - Tp0) + C_myo_tone

    A <- 1/(1+exp(-kappa_tone*S_tone))
    
    Cx <- c(0,(C[2:length(x)]-C[1:(length(x)-1)])/(L_Tub/length(x)))
    
    Ct <- -Cx*SNGFR_pre[i]*PT_frac/(pi*r_Tub^2)*S_scale - 
                2*(Vmax*C/(Km + C)*V_scale + P_Tub*(C - Ce)*P_scale)/r_Tub
    Ct[1] <- 0
    
    dDdt <- (Tp - T_pass - A*T_act)/tau_c 
    ...
    S_TGF_keep[i] <- S_TGF
    SNGFR_keep[i] <- SNGFR
    SNGFR_pre[i + tau_del] <- SNGFR
    Tp_pre[i + tau_del_m] <- Tp
    Tp_keep[i] <- Tp
    S_tone_keep[i] <- S_tone
    S_SS_keep[i] <- S_SS
  ...
  D_1 <- D_keep[samp_vec]
  C_MD_1 <- C_MD_keep[samp_vec]
  S_TGF_1 <- S_TGF_keep[samp_vec]
  SNGFR_1 <- SNGFR_keep[samp_vec]
  SNGFR_pre_1 <- SNGFR_pre
  Tp_1 <- Tp_keep[samp_vec]
  dDdt_1 <- dDdt_keep[samp_vec]
  S_tone_1 <- S_tone_keep[samp_vec]
  S_SS_1 <- S_SS_keep[samp_vec]
  
  return(list(D=D_1,
              C_MD=C_MD_1,
              S_TGF=S_TGF_1,
              SNGFR=SNGFR_1,
              SNGFR_pre=SNGFR_pre_1,
              Tp=Tp_1-Tp0,
              dDdt=dDdt_1,
              S_SS=S_SS_1,
              S_tone=S_tone_1))
...
check_takenaka <- function(D, 
  ...
  Tp <- D*(Pavg - P_ext)/2
  Tp0 <- DAA_0*(glom_SS(Pa=Paa_i, Ca=Ca_i, D=DAA_0,surr_glom_df = surr_glom_df)$Pavg - P_ext)/2
  ...
  x_AL <- seq((L_DHL + L_PT),(L_DHL + L_PT + L_Tub),(L_Tub)/1000)
  
  parameters_AL <- c(Loop_Flow = out_PT[,3][length(out_PT)/3]*A_M)
  out_AL <- ode(y = out_PT[,2][length(out_PT)/3], 
                times = x_AL, 
                func = ss_Tubule,
                parms = parameters_AL)
  C <- out_AL[,2]
  ...
  S_myo <- Splus_myo/(1+exp(-C_myo*(Tp - Tp_myo))) + Smin_myo
  S_0 <- -Splus_TGF/2 - Splus_myo/2 - Smin_myo - Smin_TGF
  ...
  S_tone <- S_TGF_func + S_myo_func# + S_0
...
tak_err <- function(P_aa_input,
  ...
  for (i in seq(P_aa_input)){
    ...
    int_high_neg <- D_try[which.min(err_x)]
    optim_dummy <- uniroot(splinefun(D_try,err_x),interval=c(int_low,int_high_neg))
...
return(list(err=err, D_zer = D_zer_out, S_TGF = C_MD_out,S_Myo = S_myo_out,
            Tp = Tp_out, S_tone=S_tone_out,S_SS=S_SS_out,SNGFR=SNGFR_out))
}
...
ss_Tubule <- function(x, Css, parameters){
  ...
  V_scaled <- Vmax*1e3    #accounts for different volume units
  P_scaled <- P_Tub
  dCssdx <- -2*pi*r_Tub*(V_scaled*Css/(Km+Css) + P_scaled*(Css - Ce))/Q
  as.list(dCssdx)
}
...
ss_Tubule_Richfield <- function(x,BB,parameters){
  ...
  #Pv_func <- Pv*(1/(1+exp(-v)) - 1/2)
  #Vmax_func <- Vmax*(1/(1+exp(-v)) - 1/2)
  
  J_v <-  Pv*(C - Ce_calc(x))
  J_s <- -J_v*C/A_M/v - Vmax*C/(C + Km)/v
...
glom_SS <- function(Pa,Ca,D,surr_glom_df){
  ...
  if(dim(df_try)[1] == 4){
    ...
    xi <- input_vec[indx_test]
    x <- df_try[,indx_test[1]]
    y <- df_try[,indx_test[2]]
